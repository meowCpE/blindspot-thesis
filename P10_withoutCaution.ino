...
void handleCommand(char *cmd) {
  ...
  // 1. GREEN RESET: When the road is clear
  ...
  // 2. RED LIGHT WARNING: Replaces CAUTION
  // This will show "SLOW DOWN" until speed data (S:xx) arrives
  if (!strcmp(cmd, "SLOW DOWN")) {
    isClear = false;
    if (!speedReceived) {
      showSlow = true; // Use this to keep the text on screen
      slowStart = millis();
  ...
  // 3. SPEED DATA: Store the incoming speed
  if (cmd[0] == 'S' && cmd[1] == ':') {
  ...
  // 4. ETA DATA: Final piece of info, then display the Speed/ETA
  if (cmd[0] == 'E' && cmd[1] == ':') {
    strncpy(currentETA, cmd + 2, sizeof(currentETA) - 1);
    currentETA[sizeof(currentETA) - 1] = '\0';
    
    showSlow = false; // Stop showing "SLOW DOWN"
    displayInfo();    // Switch to "XX km/h ETA: XXs"
  }
}
